...
CKPT_NAMES = list(CKPT_NAME_VER_DICT.keys())
...
def download_model(ckpt_name: str, model_path: str) -> bool:
    """Download model from GitHub releases if it doesn't exist locally."""
    if os.path.exists(model_path):
        return True

    if ckpt_name not in CKPT_NAME_VER_DICT:
        print(f"Comfy-VFI: Unknown model '{ckpt_name}', cannot download.")
        return False

    url = f"https://github.com/Fannovel16/ComfyUI-Frame-Interpolation/releases/download/models/{ckpt_name}"
    os.makedirs(os.path.dirname(model_path), exist_ok=True)

    try:
        print(f"Comfy-VFI: Downloading {ckpt_name} from {url}...")
        urllib.request.urlretrieve(url, model_path)
        print(f"Comfy-VFI: Successfully downloaded {ckpt_name}")
        return True
    except urllib.error.URLError as e:
        print(f"Comfy-VFI: Failed to download {ckpt_name}: {e}")
        return False
    except Exception as e:
        print(f"Comfy-VFI: Unexpected error downloading {ckpt_name}: {e}")
        return False
...
class RIFE_VFI:
    ...
    def _get_model_path(self, model_name: str) -> str:
        """获取模型文件的完整路径"""
        rife_models_folder = os.path.join(folder_paths.models_dir, "rife")
        return os.path.join(rife_models_folder, model_name)
    ...
    def vfi(
        ...
        **kwargs
        ...
        if not download_model(ckpt_name, model_path):
            raise RuntimeError(f"Model {ckpt_name} not found and could not be downloaded.")
        ...
        if cache_key not in _model_cache:
            ...
            interpolation_model.load_state_dict(torch.load(model_path, map_location=device, weights_only=False))
